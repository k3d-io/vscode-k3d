    ...
    if (target) {
        try {
            if (target.value.nodeType === "node") {
                performDeleteAgentNodeToCluster(
...
async function deleteAgentNodeToCluster(clusterName: string): Promise<void> {
    ...
    if (nodeName === undefined) {
        await vscode.window.showErrorMessage(
            `Could not get an agent node in cluster "${clusterName}"`
        );
        return;
    }
...
async function performDeleteAgentNodeToCluster(
    nodeName: string,
    clusterName: string
): Promise<void> {
    const result = await longRunning(
        `Deleting agent node "${nodeName}" from "${clusterName}"...`,
