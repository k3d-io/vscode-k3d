...
import {
    tryResolveClusterNode,
    promptCluster,
    promptNodesInCluster,
} from "./utils";
    ...
    if (target) {
        try {
            if (target.value.nodeType === "node") {
                performDeleteServerNodeToCluster(
...
async function deleteServerNodeToCluster(clusterName: string): Promise<void> {
    const nodeName = await promptNodesInCluster(
        clusterName,
        "server",
        "Server node",
    ...
    if (nodeName === undefined) {
        await vscode.window.showErrorMessage(
            `Could not get an server node in cluster "${clusterName}"`
        );
        return;
    }
...
// deleteServerByName will be invoked when users click on "Delete server"
async function performDeleteServerNodeToCluster(
    nodeName: string,
    clusterName: string
): Promise<void> {
    const result = await longRunning(
        `Deleting server node "${nodeName}" from "${clusterName}"...`,
