...
class K3dTreeDataProvider
    implements vscode.TreeDataProvider<K3dCloudProviderTreeNode>
{
    private onDidChangeTreeDataEmitter: vscode.EventEmitter<
        K3dCloudProviderTreeNode | undefined
        ...
        switch (element.nodeType) {
                        ...
                        function (cluster) {
                            ...
                            if (cluster.succeeded) {
                                ...
                                if (k3d.getClusterGrowServers(cluster.result)) {
                                    clusterTreeItem.contextValue +=
                                        " k3d.clusterServerGrowable";
                                    serversCanGrow = "yes";
                                }

                                clusterTreeItem.tooltip =
                                    new vscode.MarkdownString()
                                        .appendMarkdown(dedent`
                                <strong>k3d cluster _"${
                                    element.clusterName
                                }"_ </strong>
                                * ${
                                    cluster.result.serversRunning
                                } servers running (${
                                        cluster.result.serversCount
                                    } total) / ${
                                        cluster.result.agentsRunning
                                    } agents running (${
                                        cluster.result.agentsCount
                                    } total)
                                * growable servers: ${serversCanGrow}
                                * created at _${cluster.result.created}_
                                * load balancer: _${
                                    cluster.result.hasLoadBalancer
                                }_
                                * images volume: _${cluster.result.imageVolume}_
                                * networks: _${cluster.result.nodes
            ...
            case "node":
                return new Promise<vscode.TreeItem>((resolve) => {
                    ...
                    k3d.getClusterInfo(shell, element.clusterName).then(
                        function (cluster) {
                            const nodeTreeItem = new vscode.TreeItem(
                                element.clusterName,
                                vscode.TreeItemCollapsibleState.None
                            );
                            nodeTreeItem.contextValue = `k3d.node`;
                            nodeTreeItem.collapsibleState =
                                vscode.TreeItemCollapsibleState.None;

                            // look for this cluster in the list of current clusters
                            if (cluster.succeeded) {
                                const node = cluster.result.nodes.find(
                                    (node) => node.name === element.nodeName
                                );
                                if (node !== undefined) {
                                    // create some description (text that is displayed by the side)
                                    // and tooltip for this cluster
                                    ...
                                    switch (node.role) {
                                        case "server":
                                            ...
                                            if (
                                                ...
                                                nodeTreeItem.contextValue +=
                                                    " k3d.nodeServerRemovable";
                                            }
                                            break;

                                        case "agent":
                                            nodeTreeItem.contextValue +=
                                                " k3d.nodeAgent k3d.nodeAgentRemovable";
                                            break;
                                    }

                                    nodeTreeItem.tooltip =
                                        new vscode.MarkdownString()
                                            .appendMarkdown(dedent`
    ...
    getChildren(
        element?: K3dCloudProviderTreeNode | undefined
    ): vscode.ProviderResult<K3dCloudProviderTreeNode[]> {
        if (element) {
            switch (element.nodeType) {
                case "cluster":
...
async function getClusters(): Promise<K3dCloudProviderTreeNode[]> {
    ...
    return clusters.result.map(
        (c) => ({ nodeType: "cluster", clusterName: c.name } as const)
    );
}

// getNodesForCluster get the current list of nodes in a k3d cluster
async function getNodesForCluster(
    clusterName: string
): Promise<K3dCloudProviderTreeNode[]> {
    const clusters = await k3d.getClusters(shell);
    if (failed(clusters)) {
        return [{ nodeType: "error", diagnostic: clusters.error[0] }];
    }
    ...
    if (cluster === undefined) {
        return [
            { nodeType: "error", diagnostic: `could not find ${clusterName}` },
        ];
    }
...
async function getK3dKubeconfigYaml(
    clusterName: string
): Promise<string | undefined> {
    const kcyaml = await k3d.getKubeconfig(shell, clusterName);
    if (failed(kcyaml)) {
        vscode.window.showErrorMessage(
            `Can't get kubeconfig for k3d cluster "${clusterName}": ${kcyaml.error[0]}`
        );
        return undefined;
    }

    const originalKubeconfig = kcyaml.result;
    const distinctKubeconfig = replaceAPIServerIP(
        originalKubeconfig,
        clusterName
    );
