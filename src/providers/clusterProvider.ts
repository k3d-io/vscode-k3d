...
async function onNext(
    wizard: k8s.ClusterProviderV1.Wizard,
    _action: k8s.ClusterProviderV1.ClusterProviderAction,
    message: any
): Promise<void> {
...
async function getPage(
    defaults: settings.ClusterCreateSettings,
    sendingStep: string,
    previousData: any
): Promise<k8s.ClusterProviderV1.Sequence<string>> {
    switch (sendingStep) {
...
async function getPageCreatingCluster(
    previousData: any
): Promise<k8s.ClusterProviderV1.Observable<string>> {
    return {
        async subscribe(
            observer: k8s.ClusterProviderV1.Observer<string>
        ): Promise<void> {
            ...
            const createSettings =
                form.createClusterSettingsFromForm(previousData);

            const kubeconfig = await kubectl.getKubeconfigPath();

            const progressSteps: Observable<ProgressStep<Errorable<null>>> = k3d
                .createCluster(shell, createSettings, kubeconfig)
...
function formPage(
    ...
    const previousDataFields = Object.keys(previousData)
        ...
        .map(
            (k) =>
                `<input type='hidden' name='${k}' value='${previousData[k]}' />`
        )
        .join("\n");
