...
export async function registryRequest<T>(
    ...
    url: string,
    customOptions?: request.RequestPromiseOptions
): Promise<IResponse<T>> {
...
export async function registryTagsForImage(
    registry: string,
    namespace: string,
    repoName: string,
    regex?: RegExp,
    arch?: string,
    limit: number = MAX_TAGS_RESULTS
): Promise<Errorable<ITag[]>> {
        ...
        res = res.concat(
            response.body.results
                ...
                .filter((image) =>
                    arch
                        ? image.images.some(
                              (x) => x.architecture === getVariantForArch(arch)
                          )
                        : true
                )
        );
