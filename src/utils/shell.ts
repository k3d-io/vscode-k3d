...
export interface Shell {
    ...
    execTracking(
        ...
        args: string[],
        opts: ExecOpts
    ): rx.Observable<ProcessTrackingEvent>;
}
...
function execTracking(
    ...
    const spawnEvents = spawnrx.spawn<{
        source: "stdout" | "stderr";
        text: string;
    }>(command, args, { split: true, env: opts.env, cwd: opts.cwd });
...
export type ProcessTrackingEvent =
    | LineEvent
...
function exitCodeFrom(error: any): "no-program" | number | undefined {
    ...
    if (
        ...
        return Number.parseInt(
            (error.message as string).substring(prefix.length)
        );
    }
    return undefined;
}
